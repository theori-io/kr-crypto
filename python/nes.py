#!/usr/bin/python
# -*- coding: utf-8 -*-

class NES(object):
    block_size = 32
    key_size = 32
    EK = [0] * 416
    DEK = [0] * 416
    S = [
            
        ]
    invS = [
            

        ]
    P1 = [
            3, 1, 1, 2, 0, 0, 0, 0,
            2, 3, 1, 1, 0, 0, 0, 0,
            1, 2, 3, 1, 0, 0, 0, 0,
            1, 1, 2, 3, 0, 0, 0, 0,
            0, 0, 0, 0, 3, 1, 1, 2,
            0, 0, 0, 0, 2, 3, 1, 1,
            0, 0, 0, 0, 1, 2, 3, 1,
            0, 0, 0, 0, 1, 1, 2, 3
        ]
    invP1 = [
            9, 14, 11, 13, 0, 0, 0, 0,
            13, 9, 14, 11, 0, 0, 0, 0,
            11, 13, 9, 14, 0, 0, 0, 0,
            14, 11, 13, 9, 0, 0, 0, 0,
            0, 0, 0, 0, 9, 14, 11, 13,
            0, 0, 0, 0, 13, 9, 14, 11,
            0, 0, 0, 0, 11, 13, 9, 14,
            0, 0, 0, 0, 14, 11, 13, 9
        ]
    P2 = [
            3, 7, 5, 2, 5, 4, 2, 3,
            3, 3, 7, 5, 2, 5, 4, 2,
            2, 3, 3, 7, 5, 2, 5, 4,
            4, 2, 3, 3, 7, 5, 2, 5,
            5, 4, 2, 3, 3, 7, 5, 2,
            2, 5, 4, 2, 3, 3, 7, 5,
            5, 2, 5, 4, 2, 3, 3, 7,
            7, 5, 2, 5, 4, 2, 3, 3
        ]
    invP2 = [
            0x45, 0x72, 0x13, 0xDF, 0x54, 0x53, 0x50, 0x5A,
            0x5A, 0x45, 0x72, 0x13, 0xDF, 0x54, 0x53, 0x50,
            0x50, 0x5A, 0x45, 0x72, 0x13, 0xDF, 0x54, 0x53,
            0x53, 0x50, 0x5A, 0x45, 0x72, 0x13, 0xDF, 0x54,
            0x54, 0x53, 0x50, 0x5A, 0x45, 0x72, 0x13, 0xDF,
            0xDF, 0x54, 0x53, 0x50, 0x5A, 0x45, 0x72, 0x13,
            0x13, 0xDF, 0x54, 0x53, 0x50, 0x5A, 0x45, 0x72,
            0x72, 0x13, 0xDF, 0x54, 0x53, 0x50, 0x5A, 0x45
        ]

    def __init__(self, key):
        self.key_schedule(key)
        self.dekey_schedule()

    def mult2(self, a):
        if a & 0x80:
            return (a << 1) ^ 0x11B
        return a << 1

    def mult(self, a, b):
        x = 0
        for i in xrange(7, -1, -1):
            x = self.mult2(x)
            if b & (1 << i):
                x ^= a
        return x

    def multMatrix(self, vIn, M):
        vOut = [0] * 8
        for i in xrange(8):
            for j in xrange(8):
                vOut[i] = vOut[i] ^ self.mult(vIn[j], M[i*8 + j])
        return vOut

    def round(self, state, RK, S, P):
        for i in xrange(32):
            state[i] = S[state[i]]
        for j in xrange(4):
            for i in xrange(j):
                state[j*8 + i], state[i*8 + j] = state[i*8 + j], state[j*8 + i]
                state[j*8 + 4 + i], state[i*8 + 4 + j] = state[i*8 + 4 + j], state[j*8 + 4 + i]
        if P:
            for j in xrange(4):
                row = state[j*8:j*8+8]
                tmp = self.multMatrix(row, P)
                for i in xrange(8):
                    state[j*8 + i] = tmp[i]
        for i in xrange(32):
            state[i] = state[i] ^ RK[i]

    def process(self, data, EK, S, P1, P2):
        state = [0] * 32
        for i in xrange(32):
            state[i] = data[i] ^ EK[i]
        for n in xrange(1, 12):
            if n % 2 == 1:
                self.round(state, EK[n*32:n*32+32], S, P1)
            else:
                self.round(state, EK[n*32:n*32+32], S, P2)
        self.round(state, EK[12*32:13*32], S, None)
        for i in xrange(32):
            data[i] = state[i]

    def encrypt(self, data):
        self.process(data, self.EK, self.S, self.P1, self.P2)
        return data

    def decrypt(self, data):
        self.process(data, self.DEK, self.invS, self.invP1, self.invP2)
        return data
    
    def key_schedule(self, key):
        k = list(key)
        tmp = [0] * 8
        for n in xrange(13):
            j = n % 8
            j1, j2, j3, j4 = (j+1) % 8, (j+2) % 8, (j+3) % 8, (j+4) % 8
            for i in xrange(4):
                tmp[i] = self.invS[k[i + j*4]]
            tmp2 = self.multMatrix(tmp, self.invP1)
            for i in xrange(4):
                k[i + j*4] = tmp2[i] ^ k[i + j1*4]
            for i in xrange(4):
                self.EK[n*32 + i + 0] = k[i + j*4]
                self.EK[n*32 + i + 4] = k[i + j*4] ^ k[i + j2*4]
                self.EK[n*32 + i + 8] = k[i + j*4] ^ k[i + j3*4]
                self.EK[n*32 + i + 12] = k[i + j*4] ^ k[i + j4*4]
            j = (n + 4) % 8
            j1, j2, j3, j4 = (j+1) % 8, (j+2) % 8, (j+3) % 8, (j+4) % 8
            for i in xrange(4):
                tmp[i] = self.invS[k[i + j*4]]
            tmp2 = self.multMatrix(tmp, self.invP1)
            for i in xrange(4):
                k[i + j*4] = tmp2[i] ^ k[i + j1*4]
            for i in xrange(4):
                self.EK[n*32 + 16 + i + 0] = k[i + j*4]
                self.EK[n*32 + 16 + i + 4] = k[i + j*4] ^ k[i + j2*4]
                self.EK[n*32 + 16 + i + 8] = k[i + j*4] ^ k[i + j3*4]
                self.EK[n*32 + 16 + i + 12] = k[i + j*4] ^ k[i + j4*4]

    def dekey_schedule(self):
        for n in xrange(13):
            eRow = self.EK[(12-n)*32:(12-n)*32+32]
            if n == 0 or n == 12:
                for i in xrange(32):
                    self.DEK[n*32 + i] = eRow[i]
            elif n % 2 == 1:
                for j in xrange(4):
                    tmp = self.multMatrix(eRow[j*8:j*8+8], self.invP1)
                    for k in xrange(8):
                        self.DEK[n*32 + j*8 + k] = tmp[k]
            else:
                for j in xrange(4):
                    tmp = self.multMatrix(eRow[j*8:j*8+8], self.invP2)
                    for k in xrange(8):
                        self.DEK[n*32 + j*8 + k] = tmp[k]
